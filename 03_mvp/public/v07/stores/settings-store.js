// settings-store.js — page-clone du pattern decisions (S6.11-EE)
...
class PageStore extends Store {
  ...
  async load() {
    ...
    try {
      ...
      const items = Array.isArray(data) ? data : (data['settings'] || data.items || []);
      this.setState({ items, loading: false });
    } catch (e) {
      console.error('[settings-store] load failed', e);
