// index-store.js — page-clone du pattern decisions (S6.11-EE)
...
class PageStore extends Store {
  ...
  async load() {
    ...
    try {
      const r = await fetch('/api/cockpit/today');
      if (!r.ok) throw new Error('HTTP ' + r.status);
      const data = await r.json();
      const items = Array.isArray(data) ? data : (data['cockpit'] || data.items || []);
      this.setState({ items, loading: false });
    } catch (e) {
      console.error('[index-store] load failed', e);
